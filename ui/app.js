...
function renderAsset(asset){
  ...
  const roomList=servedRooms.length?`<div class="served-rooms"><strong>Rooms / labs served</strong><p>“Room” and “Lab” are interchangeable in FacilityIQ. Each listed AHU area has a dedicated CAV/VAV terminal, and every VAV has a heating valve. Terminal type and tag remain to be confirmed.</p><div>${servedRooms.map(room=>`<span>${esc(room)}</span>`).join("")}</div></div>`:"";
